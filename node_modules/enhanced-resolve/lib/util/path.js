 ...
 * @returns {PathType} AbsoluteWin for `\\?\â€¦` / `\\.\â€¦`, otherwise Normal
 */
...
const getType = (maybePath) => {
    ...
    // DOS device paths (`\\?\â€¦`, `\\.\â€¦`) are handled in a cold helper so
    // this function stays small â€” inlining the full check here regressed
...
/**
 * Whether `request` is a relative request â€” i.e. matches `^\.\.?(?:\/|$)`.
 *
 ...
 * anchoring effect â€” a cheap `startsWith` plus a separator char check on the
...
const isSubPath = (parentPath, childPath) => {
    ...
    if (lastChar === CHAR_SLASH || lastChar === CHAR_BACKSLASH) {
        // Parent already ends with a separator â€” a plain prefix test is enough.
 ...
 * nodejs/node#17658) â€” so a directory literally named `file:` is never
 * mistaken for a URL.
