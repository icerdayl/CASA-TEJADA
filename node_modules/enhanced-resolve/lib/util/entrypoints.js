 ...
 * first observation â€” i.e. parsed `package.json` fields and the nested
...
function getFieldKeyInfos(field) {
    ...
    for (let i = 0; i < keys.length; i++) {
        ...
        // we avoid the full-string scan that `lastIndexOf` would do â€” and the
 ...
 * `conditionNames` â€” that's applied separately by `conditionalMapping`),
...
function findMatch(request, field) {
    ...
    if (perRequest === undefined) {
        ...
        // `computeFindMatch` only ever returns `MatchTuple | null` â€” never
        // `undefined` â€” and `Map.set(k, null)` then `Map.get(k)` returns
 ...
 * cache-hit path be a single `WeakMap.get()` â€” we distinguish
 ...
 * depends only on the mapping object (immutable â€” owned by a parsed
