 ...
 * @property {string[]} contextList precomputed `Object.keys(allContexts)` â€” read-only; used on the `_selectPathsDataForContext` hot path
...
class StackEntry {
     ...
     * ~1 MiB / 200 resolves on stack-churn â€” V8 keeps a polymorphic
     ...
     * against the pre-5.21 `Set<string>` shape â€” e.g.
     * `[...resolveContext.stack].find(a => a.includes("module:"))` â€” keep
 ...
 * @property {StackEntry | Set<string>=} stack tip of the resolver call stack (a singly-linked list with Set-like API). For instance, `resolve â†’ parsedResolve â†’ describedResolve`. Accepts a legacy `Set<string>` for back-compat with older callers; it is normalized internally without a hot-path branch.
...
class Resolver {
    ...
    resolve(context, path, request, resolveContext, callback) {
        ...
        // req, resolveCtx, cb)`) â€” every call from `resolveSync` /
    ...
    doResolve(hook, request, message, resolveContext, callback) {
        ...
        if (hook.isUsed()) {
            ...
            // literal â€” `createInnerContext` reads from the parent and
    ...
    parse(identifier) {
        ...
        if (part.request.length > 0) {
            ...
            // `\0#â€¦` escape path produces a fresh `part.request` and falls
            ...
            // `isDirectory` is just `endsWith("/")` â€” inline so `parse()`
