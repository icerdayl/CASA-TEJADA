...
// lets the cache-hit path be a single `Map.get()` â€” we distinguish
 ...
 *
 * The outer key is the `TsconfigPathsMap` itself â€” rebuilt on every
 * tsconfig change â€” so a `WeakMap` lets the inner map be collected
...
// `./src/*` â†’ `./src`). Hoisted so we don't allocate a fresh regex per
// path entry on every tsconfig load â€” and so the same regex object can be
...
module.exports = class TsconfigPathsPlugin {
    ...
    _selectPathsDataForContext(requestPath, tsconfigPathsMap) {
        ...
        // â€” that's the monorepo / project-references shape where the
        ...
        // lookup to after we know the context actually matches â€” non-matches
