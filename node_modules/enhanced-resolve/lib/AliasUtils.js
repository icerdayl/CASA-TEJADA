 ...
 * @property {string} nameWithSlash name + "/" â€” precomputed to avoid per-resolve concat
 ...
 * @property {number} firstCharCode first character code of `name` â€” used as a cheap screen on the hot path. `-1` indicates "matches any first char" (empty wildcard prefix).
 * @property {boolean} arrayAlias true when `alias` is an array â€” precomputed so the hot path skips `Array.isArray`
 */
 ...
 * â€” each bucket preserves declaration order among its members. The
 ...
 * The `useBuckets` flag is true only when bucketing would actually help â€”
 ...
 * all share one first char â€” the bucket lookup adds overhead without
 ...
 * Called once per plugin apply â€” the returned structure is stable for the
...
function compileAliasOptions(resolver, options) {
    ...
    for (let i = 0; i < options.length; i++) {
        ...
        // wildcard prefix) can match arbitrary first chars â€” encode that as -1.
        let firstCharCode;
    ...
    // `chain-0 -> chain-1 -> â€¦` rewrites), the resolve-time `Map.get`
    // does no discrimination â€” every request lands in that one bucket
    // or in nothing â€” and the lookup is overhead compared to walking
...
function aliasResolveHandler(
    ...
    // entries (e.g. monorepos with generated alias lists) â€” see the
    ...
    // only one first-char bucket â€” e.g. a long chain of `chain-N`
    // aliases that all start with the same char â€” every request lands
