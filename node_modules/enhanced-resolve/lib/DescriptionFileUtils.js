 ...
 *
 * Any single-character directory is treated as a root â€” `directory.length
 * <= 1` collapses the `"/"`, `"\\"` and `""` branches into one compare.
 * Without the `"\\"` case, `cdUp("\\")` (reached from a UNC root or a DOS
 * device path like `\\?\â€¦`) would return itself via `slice(0, i || 1)`
...
function loadDescriptionFile(
    ...
    // across every level â€” the previous implementation re-allocated both
