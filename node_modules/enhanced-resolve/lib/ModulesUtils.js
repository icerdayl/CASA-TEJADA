 ...
 * `ancestor Ã— directory` is deterministic per request.path, and many resolves
 * share the same starting directory (sibling files in one project, loops over
 * a batch of imports, etc.) â€” caching avoids the `getPaths` regex split plus
 * `len(paths) Ã— len(directories)` join calls per resolve.
 *
 ...
 * `resolver.pathCache`) so the pathCache's hidden-class shape is unchanged â€”
...
function modulesResolveHandler(
    ...
    // function) so the cache-hit path â€” which is the vast majority of
    // invocations â€” stays a single WeakMap + Map lookup with no function-call
    ...
    // can have up to `paths.length Ã— directories.length` entries (e.g. 36
    // for an 8-deep source dir Ã— 4-module config), and concatenating the
    // same `./${request.request}` string on every iteration is wasted
    // work â€” it's constant for the whole fan-out.
